...
def test_cache_persistence_across_days():
    url = "https://example.com/test-expiration-api"
    headers = {"X-Test": "789"}
    
    mock_response = MagicMock()
    mock_response.read.return_value = b"Cached Data"
    ...
    with patch("urllib.request.urlopen") as mock_urlopen:
        ...
        with patch("backend.utils.datetime") as mock_datetime:
            ...
            # _get_cache_path matches existing file by hash suffix, avoiding new network calls
            mock_urlopen.reset_mock()
            data2 = fetch_url_with_retry(url, headers=headers, use_cache=True)
            assert data2 == b"Cached Data"
            assert mock_urlopen.call_count == 0
