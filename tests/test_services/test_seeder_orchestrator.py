...
def test_seed_world_cup_does_not_call_api_football(db_session, monkeypatch):
    """World Cup seed uses the static dataset even when a retired API-Football key is present."""
    static_elo = json.loads(WC_JSON.read_text(encoding="utf-8"))["elo_ratings"]
    monkeypatch.setattr("backend.services.seeder.fetch_current_elo_ratings", lambda: dict(static_elo))
