...
from backend.database import get_db, init_db, Fixture, Team, Player
from backend.scoring import update_fixture_score
from backend.ingestor import seed_database

app = FastAPI(title="Football Match Watchability Index")
...
def update_weights(weights: WeightUpdate, db: Session = Depends(get_db)):
    ...
    # Recalculate scores for all fixtures
    fixtures = db.query(Fixture).all()
    for fixture in fixtures:
        update_fixture_score(fixture, db, scoring_weights)
    db.commit()
...
def refresh_data(db: Session = Depends(get_db)):
    seed_database(db)
    # Re-apply current weights if they aren't default
    fixtures = db.query(Fixture).all()
    for fixture in fixtures:
        update_fixture_score(fixture, db, scoring_weights)
    db.commit()
...
def get_fixtures(tz: str = "UTC", db: Session = Depends(get_db)):
    """
    Returns fixtures grouped by Today (June 11), Tomorrow (June 12), and This Week (June 13+),
    sorted by watchability score descending, converted to the requested timezone.
    """
    ...
    for f in fixtures:
        ...
        # Build enriched team/player data for tooltips or details modal
        home_team = db.query(Team).filter(Team.name == f.home_team_name).first()
        away_team = db.query(Team).filter(Team.name == f.away_team_name).first()
        
        home_players = db.query(Player).filter(Player.team_name == f.home_team_name).all()
        away_players = db.query(Player).filter(Player.team_name == f.away_team_name).all()
        ...
        # Grouping based on the converted day (World Cup kickoff starts June 11, 2026)
    ...
    # Sort each group by watchability score descending
    ...
    # Keep only the top 5 matches for the third panel
...
@app.get("/api/fixtures/recommended")
def get_recommended_fixtures(tz: str = "UTC", db: Session = Depends(get_db)):
    """
    Returns fixtures with watchability score >= 75%, localized to the target timezone.
    """
    from zoneinfo import ZoneInfo
    try:
        target_tz = ZoneInfo(tz)
    ...
    fixtures = db.query(Fixture).filter(Fixture.watchability_score >= 75).all()
    ...
    for f in fixtures:
        ...
        recommended_list.append(fixture_data)
        
    return recommended_list


def calculate_standings_internal(group_letter: str, db: Session) -> list:
    teams = db.query(Team).filter(Team.group_name == group_letter).all()
    standings = []
    for t in teams:
        standings.append({
            "name": t.name,
...
def get_country_details(country_name: str, tz: str = "UTC", db: Session = Depends(get_db)):
    ...
    team = db.query(Team).filter(Team.name == country_name).first()
    if not team:
        raise HTTPException(status_code=404, detail="Country not found")
        
    group_standings = calculate_standings_internal(team.group_name, db)
    rank = 1
    ...
    players = db.query(Player).filter(Player.team_name == country_name).order_by(Player.form_score.desc()).limit(3).all()
    ...
    future_fixtures = db.query(Fixture).filter(
        (Fixture.status != "Finished") & 
        ((Fixture.home_team_name == country_name) | (Fixture.away_team_name == country_name))
    ).all()
    ...
    for f in future_fixtures:
        ...
        h_team = db.query(Team).filter(Team.name == f.home_team_name).first()
        a_team = db.query(Team).filter(Team.name == f.away_team_name).first()
        
        group_letter = h_team.group_name if h_team else None
        display_stage = f"Group {group_letter}" if f.stage == "Group Stage" and group_letter else f.stage
        
        future_matches_data.append({
            "id": f.id,
            "home_team": {
                "name": f.home_team_name,
                "elo": h_team.elo if h_team else 1500,
                "form_score": h_team.form_score if h_team else 50.0,
                "win_streak": h_team.win_streak if h_team else 0
            },
            "away_team": {
                "name": f.away_team_name,
                "elo": a_team.elo if a_team else 1500,
                "form_score": a_team.form_score if a_team else 50.0,
                "win_streak": a_team.win_streak if a_team else 0
            },
            "date": f.date,
            "formatted_time": dt_tz.strftime("%H:%M"),
            "formatted_date": dt_tz.strftime("%B %d, %Y"),
...
@app.get("/api/group/{group_letter}")
def get_group_details(group_letter: str, tz: str = "UTC", db: Session = Depends(get_db)):
    from zoneinfo import ZoneInfo
    try:
        target_tz = ZoneInfo(tz)
    except Exception:
    ...
    teams_count = db.query(Team).filter(Team.group_name == group_letter).count()
    if teams_count == 0:
        raise HTTPException(status_code=404, detail="Group not found")
        
    standings = calculate_standings_internal(group_letter, db)
    ...
    for f in fixtures:
        ...
        display_stage = f"Group {group_letter}" if f.stage == "Group Stage" else f.stage
...
def simulate_group_stage(db: Session) -> dict:
    ...
    fixtures = db.query(Fixture).filter(Fixture.stage == "Group Stage").all()
    for f in fixtures:
...
@app.get("/api/bracket")
def get_bracket_predictions(db: Session = Depends(get_db)):
    ...
    def play_match(t1_name, t2_name):
        t1 = db.query(Team).filter(Team.name == t1_name).first()
        t2 = db.query(Team).filter(Team.name == t2_name).first()
