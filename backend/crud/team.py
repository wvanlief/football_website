...
def get_teams_by_group(db: Session, group_name: str) -> list[Team]:
    return db.query(Team).join(TournamentTeam).filter(TournamentTeam.group_name == group_name).all()

def count_teams_in_group(db: Session, group_name: str) -> int:
    return db.query(Team).join(TournamentTeam).filter(TournamentTeam.group_name == group_name).count()
